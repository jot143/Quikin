...
function checkVariantLimit(from, toastAlreadyShown = false) {
  ...
  if (from == "sizes" || from == "colors") {
    ...
    if (variantCount > maxVariantLimit) {
      ...
      console.log(
        `Max ${from} limit Still There. (${maxVariantLimit} Variants)`
      );
      return true;
    }
  }
...
async function editProductNew(mockupId) {
  ...
  if (success) {
    ...
    initPickr();
    initDropZone();
    updateRightSideContainerClass("mockup2");
    ...
    if (window.mockup_id == 16 || mockupId == "16") {
      //For Color Coffee Mug - We will hide the default color selection select2
...
async function initMockup(mockupId) {
  ...
  const jsonFilePath =
    baseUrl +
    `/assetsroot/admin/mockups/${mockupId}/${mockupId}.json?v=${timestamp}`;
  const jsonDesignPath =
    baseUrl +
    `/assetsroot/admin/mockups/${mockupId}/${mockupId}_design.json?v=${timestamp}`;
  const qPricePath =
  ...
  try {
    ...
    eQikinkElementData = processJsonData(jsonData, ["Shirt", "ShadowFront"]);
    window.ShirtElementSources = JSON.stringify(eQikinkElementData["Shirt"]);
...
function initDropZone() {
  ...
  if (
    dropzoneElement &&
    Dropzone.instances.some((instance) => instance.element === dropzoneElement)
  ) {
    console.log(
    ...
    var dropzone = new Dropzone(".dropzone", {
      ...
      init: function () {
        this.on("sending", function (file, xhr, formData) {
          ...
          var printtype = $('input[name="printingOption"]:checked').val();
...
async function initMockupScripts(mockupId) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch AOP products list: ${response.status}`);
    }
  ...
  if (window.yourDesigner && window.yourDesigner !== "undefined") {
    ...
    $yourDesigner.on("productCreate", function (evt) {
      ...
      Promise.all(asyncTasks)
        .then(() => {
          ...
          $("#colors-container").find("button:first-of-type").trigger("click");
        })
        .catch((error) => {
    ...
    $yourDesigner.on("elementModify", function (event, element, position) {
      if (element.type === "image" && element.title === "Design") {
        ...
        if (
          !window.disableModifyValidation &&
          $(".rightsidecontainer").hasClass("mockup1")
        ) {
    ...
    function resetFancyProductDesigner() {
      if (!yourDesigner || !yourDesigner.viewInstances) {
        console.error("Fancy Product Designer instance or views not found.");
        return;
      }
    ...
    $(".mcontainer").on("change", 'input[name="printingOption"]', function () {
    ...
    $(".mcontainer").on("change", "#plain_order_check", function () {
      if (this.checked) {
        ...
        if ($(".basePricePerSize").length) {
          $("p.basePricePerSize").attr("data-handling", plain_handling);
    ...
    $(".select-color").on("change", function (e) {
      ...
      const colorButton = $(`.color-button[bg-color-id="${selectedValue}"]`);
    ...
    function onElementModify(event, element, position) {
      ...
      let isOutOfBounds = {
        ...
        right: elementBoundingBox.right > boundingBox.x + boundingBox.width,
        bottom: elementBoundingBox.bottom > boundingBox.y + boundingBox.height,
      };
      if (
        isOutOfBounds.left ||
        isOutOfBounds.top ||
        ...
        if (isOutOfBounds.left) {
          element.left = boundingBox.x + (element.scaleX * element.width) / 2;
        }
        if (isOutOfBounds.top) {
          element.top = boundingBox.y + (element.scaleY * element.height) / 2;
        }
...
function changeTshirtColorAcc(hex, id, prefix = "") {
  ...
  if (typeof elementSourcesJSON[vindex][sourceToCheck] !== "undefined") {
    ...
    if (
      window.mockup_id == "148" ||
      elementSourcesJSON[vindex]?.designAtTop === 1
    ) {
